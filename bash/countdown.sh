...
#Task 1 - Adding trap for interrupt and quitting signals

trap interrupt INT
trap quitting QUIT

# using interrupt function reset count to max

function interrupt {
sleepCount=$numberOfSleeps

#giving message that you cannot interrupt the count
echo "You cannot interrupt the count"
}

#now using quit function
function quit {

stty sane
#giving message to user you found a secret to get out
echo"you found the way to get out of count and exit immediately"
exit 0
}
