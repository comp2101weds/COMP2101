...
# using case option for recognizing -h, -v, and -d
case $1 in
  -h)
  echo "-h is for help"
  ;;
  ...
  case "$2" in
    [1-9] )
    echo "-d is for debug, level $2"
    shift
    ;;
    *)
#error will be print if user gave option without sigle digit number from 1 to 9
    echo "-d is recognized from number [1-9] "
    shift
  esac
  ;;
  *)
  error=$1
  echo "should be an one digit number"
  shift
  ;;
esac
